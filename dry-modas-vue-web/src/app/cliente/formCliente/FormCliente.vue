    ...
    methods: {
        ...
        criarCliente() {
            const cliente = {
                ...
                vendas: this.vendas? this.vendas : []
            };
            if(cliente.id){
                this.atualizarCliente(cliente)
            }else{
                this.direcionarCriacaoCliente(cliente)
            }
        ...
        direcionarCriacaoCliente(cliente){
            if (cliente.vendas.length === 0) {
                console.log("s/VENDAS");
        ...
        buscarVendas() {
            this.$store.dispatch('addRequest');
            VendaService.listarVenda(this.id,0, 10)
                .then((res) => {
                    if (res.success === true) {
                        this.vendas = res.data.vendas;
                    } else {
